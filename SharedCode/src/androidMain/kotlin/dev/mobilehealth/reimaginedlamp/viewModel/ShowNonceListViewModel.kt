...
class ShowNonceListViewModel : ViewModel() {
    ...
    val activeNonces =
        dailyNonces.map { nonces -> nonces.filter { nonce -> nonce.activeInterval.contains(DateTime.now()) } }
...
//    })
    ...
    fun dailyNoncesLiveData() : LiveData<List<Nonce>> {
        var sharedNonceRepo = UserNonceRepository(userId!!, firestore)
        return sharedNonceRepo.getNoncesLiveData(5)
    }
